...
const LineGraphVisualization = z.object({
  ...
  x_to_percentage: z.boolean().default(false).describe("True if the x-axis values should be converted to percentages"),
  y_to_percentage: z.boolean().default(false).describe("True if the y-axis values should be converted to percentages"),
  insights: z.array(z.string()).describe("Key insights derived from the visualization")
});
...
const BarChartVisualization = z.object({
  ...
  orientation: z.enum(["vertical", "horizontal"]).describe("Orientation of the bar chart"),
  stacked: z.boolean().default(false).describe("True if the bar chart should be stacked"),
  insights: z.array(z.string()).describe("Key insights derived from the visualization")
});
...
const AreaChartVisualization = z.object({
  ...
  x_axis: z.string().describe("The exact name of the column to be used for the x-axis"),
  y_axis: z.string().describe("The exact name of the column to be used for the y-axis"),
  stacked: z.boolean().default(false).describe("True if the area chart should be stacked"),
  insights: z.array(z.string()).describe("Key insights derived from the visualization")
});

const DataSummary =  z.object({
  summary: z.string().describe("A brief overview summarizing the overall trends in the data"),
  key_metrics: z.array(
    z.object({
      metric_name: z.string().describe("The name of the metric, e.g., 'Total Sales'"),
      value: z.number().describe("The numerical value of the metric"),
      unit: z.string().optional().describe("Optional measurement unit, e.g., 'USD', 'kg'")
    })
  ).describe("A list of important quantitative metrics extracted from the data"),
  insights: z.array(z.string()).describe("A list of qualitative insights derived from the analysis"),
  recommendations: z.array(z.string()).optional().describe("Optional actionable recommendations based on the analysis")
});

 const Report = z.object({
  title: z.string().describe("The title of the report"),
  report: z.string().describe("A brief summary of the analysis structered in clear form and in .md format"),
});

// Main Visualization Types
const VisualizationType = z.discriminatedUnion("chart_type", [
...
// **New: Visualization List Schema**
 const VisualizationList = z
  ...
  .describe("A list of visualization configurations, each defining a specific way to display the data.");
